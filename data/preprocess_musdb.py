...
sys.path.append('../')
from audio_spectrogram_conversion import audio_to_spectrogram


def slice_audio_file_to_chunks(file_path, sample_rate=22050, chunk_length=5):
    """
    Takes an audio file and converts it to a list of chunks of size chunk_length.
    ...
    Returns:
        list: List of chunks of size chunk_length.
...
def process_file(file_path, dest_folder, counter):
    """
    Processes file as described in the thesis and saves processed chunks into destination folder.

    Args:
        file_path (str): Input file path.
        dest_folder (Path): Destination folder path.
        counter (int): Counter for keeping track in order to list pre-processed data points.
...
def create_spectrogram_dataset(input_folder, output_folder, split_ratios):
    """
    Iterates over files and creates pre-processed spectrogram dataset.

    Args:
        input_folder (str): Unprocessed MUSDB18 data as downloaded.
        output_folder (str): Destination folder path.
        split_ratios (dict): Dictionary with split ratios and keys ['train', 'val', 'test'].
    """
